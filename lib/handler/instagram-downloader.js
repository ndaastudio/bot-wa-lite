...
const fs = require("fs");
const { formatNumbers } = require("./formatter");

dotenv.config();

const cookieIg = process.env.INSTAGRAM_COOKIES;

function feedReelDown(url, client, message) {
  ...
  ig.fetchPost(url)
    .then(async (res) => {
      ...
      const content = `Feeds/Reels *${username}* akan dikirim, mungkin membutuhkan waktu beberapa menit. Mohon ditunggu...`;
    ...
    .catch(() => {
      const content = "Link Instagram tidak valid atau akun private";
      message.reply(content);
    });
}

async function storiesDown(id, client, message) {
  ...
  ig.fetchStories(id)
    .then(async (res) => {
      ...
      const content = `IG Stories *${username}* akan dikirim, mungkin membutuhkan waktu beberapa menit. Mohon ditunggu...`;
      await client.sendMessage(message.from, content);
    ...
    .catch(() => {
      const content =
        "Username Instagram tidak valid, akun private, atau pemilik akun tidak membuat stories";
      message.reply(content);
    });
}
